...
import { getAdminMonthlyTimeLogs } from "../actions";
import { AdminDayDetailsModal } from "./AdminDayDetailsModal";
...
export function AdminCalendarGrid({
  ...
  useEffect(() => {
    loadMonthlyLogs();
  }, [currentDate, projectId, orgId]);

  const loadMonthlyLogs = async () => {
    setLoading(true);
    const year = currentDate.getFullYear();
    const month = currentDate.getMonth();
    const data = await getAdminMonthlyTimeLogs(year, month, projectId, orgId);
    setLogs(data);
    setLoading(false);
  };
