...
export function ProfileForm({ user, availableManagers }: ProfileFormProps) {
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setMessage({
        type: "success",
        text: "Foto enviada e salva com sucesso!"
      });
    } catch (err: any) {
      setMessage({
        type: "error",
        text: err.message || "Erro ao fazer upload da foto."
      });
    } finally {
    ...
    <form onSubmit={handleSubmit} className="space-y-6 max-w-4xl">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Esta foto será exibida no seu perfil e nas tasks do sistema.
          </CardDescription>
        </CardHeader>
        <CardContent className="flex items-center gap-6">
          ...
          <div>
            ...
            <p className="text-xs text-zinc-500 mt-2">
              Formatos aceitos: JPG, PNG ou WEBP (Max 2MB).
            </p>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Atualize seu nome, cargo e estrutura de gestão.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            <label className="text-sm font-medium mb-2 block">
              Gerenciado por (Gestor)
            </label>
            <Select
              value={managerId}
              onValueChange={(val) => setManagerId(val || "none")}
            >
